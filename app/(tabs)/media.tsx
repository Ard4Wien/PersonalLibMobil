...
const TYPE_FILTERS = [
    { label: 'Tümü', value: 'ALL', icon: Film },
    { label: 'Filmler', value: 'MOVIE', icon: Film },
    { label: 'Diziler', value: 'SERIES', icon: Tv },
];
...
export default function MediaScreen() {
    ...
    const confirmDelete = async () => {
        ...
        try {
            ...
            showToast(`${deleteOpts.type === 'movie' ? 'Film' : 'Dizi'} silindi`, 'success');
    ...
    return (
        <View className="flex-1 bg-background pt-16">
            <PageHeader
                title="Medya Arşivi"
                subtitle={`${(movies?.length || 0) + (series?.length || 0)} içerik`}
                icon={Clapperboard}
                rightAction={
                    <>
                        <TouchableOpacity
            ...
            <ScrollView horizontal showsHorizontalScrollIndicator={false} className="px-6 mb-6 max-h-12">
                <View className="flex-row space-x-2 mr-6">
                    {TYPE_FILTERS.map((f) => (
                        ...
                        >
                            <f.icon size={16} color={activeType === f.value ? 'white' : '#94a3b8'} className="mr-2" />
                ...
                {(!movies?.length && !series?.length) ? (
                    <View className="items-center justify-center py-20">
                        <View className="bg-slate-900/50 p-6 rounded-full mb-4">
                            <Clapperboard size={48} color="#64748b" />
                        </View>
                        <Text className="text-text-secondary text-lg">Henüz içerik eklenmemiş</Text>
                    </View>
                ) : (
                    <View className="flex-row flex-wrap justify-between pb-10">
                        {(activeType === 'ALL' || activeType === 'MOVIE') && movies?.map((movie) => (
                            <MediaCard
                                ...
                                onDelete={() => handleDelete(movie.id, movie.title || (movie as any).name || 'Film', 'movie')}
                            />
                        ))}
                        {(activeType === 'ALL' || activeType === 'SERIES') && series?.map((s) => (
                            <MediaCard
                                ...
                                onDelete={() => handleDelete(s.id, s.title || (s as any).name || 'Dizi', 'series')}
                            />
                        ))}
                    </View>
                )}
            ...
            <DeleteConfirmModal
                ...
                title={`${deleteOpts.type === 'movie' ? 'Filmi' : 'Diziyi'} Sil`}
                message={`"${deleteOpts.title}" ${deleteOpts.type === 'movie' ? 'filmini' : 'dizisini'} kütüphanenizden kaldırmak istediğinize emin misiniz?`}
